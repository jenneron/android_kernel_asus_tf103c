...
#include <linux/proc_fs.h>	// Factory
...
//Factory>>
static const struct file_operations bcm2079x_test_fops;
static struct proc_dir_entry *bcm2079x_test_entry = NULL;
static struct delayed_work bcm2079x_detect_work;

static void bcm2079x_detect_worker(struct work_struct *work)
{
	...
	if (adapter) {
		...
		if ( (r!=1)&&(retry<=3) ) {	
			retry++;
			schedule_delayed_work(&bcm2079x_detect_work, HZ);
		} else {
			if (r == 1) {
				bcm2079x_test_entry = proc_create("NFCTest", 0666, NULL, &bcm2079x_test_fops); 
			}
			gpio_set_value(102+28+9, 0);	//Disable NFC
			cancel_delayed_work(&bcm2079x_detect_work);
			flush_delayed_work(&bcm2079x_detect_work);
		} 
	}
}
//Factory<<
...
//Factory>>
	...
	INIT_DELAYED_WORK(&bcm2079x_detect_work, bcm2079x_detect_worker);
	schedule_delayed_work(&bcm2079x_detect_work, HZ);
//Factory<<
