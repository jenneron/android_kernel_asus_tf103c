...
//	printk("jjt elan_i2c_report_standard\n");
	return;
}
...
int elan_i2c_check_packet(u8 *packet)
{
	...
	printk("jjt check packet length=%x(%x), report_id=%x(%x)\n",length, ETP_REPORT_LENGTH,report_id,ETP_REPORT_ID);

	if (length == ETP_REPORT_LENGTH && report_id == ETP_REPORT_ID){
		printk("jjt elan_i2c_check_packet absolute mode\n");
		return 0;
	}else if (length == 7 && report_id == 0x01){
		printk("jjt elan_i2c_check_packet standard mode\n");
		return 1;
	}else{
		printk("jjt elan_i2c_check_packet error\n");
		return -1;
	}
}
