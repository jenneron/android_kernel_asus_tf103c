...
void usb2phy_eye_optimization(struct dwc_otg2 *otg)
{
	struct intel_dwc_otg_pdata *data;
	struct usb_phy *phy;
...
EXPORT_SYMBOL_GPL(usb2phy_eye_optimization);
			...
			dwc3_intel_byt_get_charger_type_asus(struct dwc_otg2 *otg)
{
	...
	if (!phy) {
		...
		PowerStateOk = 1;
		printk("tant --->POWER_SUPPLY_CHARGER_TYPE_NONE\n");
		return POWER_SUPPLY_CHARGER_TYPE_NONE;
	}
...
enum power_supply_charger_cable_type
	GetPowerState(void)
{
	printk("GetPowerState:%d\n",Power_State);
	return Power_State;
}
EXPORT_SYMBOL(GetPowerState);
//-----------------------------------
...
struct dwc3_otg_hw_ops dwc3_intel_byt_otg_pdata = {
	...
	.get_charger_type = dwc3_intel_byt_get_charger_type_asus,
