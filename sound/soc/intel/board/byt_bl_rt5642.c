...
#define TF103C_BOARD_ER1				0
#define TF103C_BOARD_ER2				2
#define TF103C_BOARD_ER3				3
#define TF103C_BOARD_PR					1
...
{
	int gpio0p2_v1,gpio0p5_v2;
	intel_mid_pmic_writeb(0x2d,0x14);//PMIC GPIO0P2 CTLO
	intel_mid_pmic_writeb(0x30,0x14);//PMIC GPIO0P5 CTLO
	msleep(10);
	gpio0p2_v1 = intel_mid_pmic_readb(0x35) & 0x01;//PMIC GPIO0P2 CTLI
	gpio0p5_v2 = intel_mid_pmic_readb(0x38) & 0x01;//PMIC GPIO0P5 CTLI
	return (gpio0p2_v1 << 1 | gpio0p5_v2);
}
...
static int snd_byt_mc_probe(struct platform_device *pdev)
{
	...
	//steve_chen@asus.com++
	if(get_Board_Type() == TF103C_BOARD_PR)
	{
	...
	if (INTEL_MID_BOARD(3, TABLET, BYT, BLK, PRO, 8PR1)) {
		...
		if (jd_gpio >= 0 && get_Board_Type() != TF103C_BOARD_ER1) {
